...
// 4. AUTOCOMPLETE PERANGKAT DAERAH
...
<div class="p-3 p-md-4">
    <div v-if="!showModal && !showPreviewModal">
        ...
        <div class="d-flex flex-column flex-md-row justify-content-between align-items-md-center mb-4 gap-3">
            <div>
                <h3 class="fw-bold text-danger mb-1">Data Inpassing</h3>
                <p class="text-muted small mb-0">Manajemen penyesuaian gaji (Inpassing) ASN.</p>
            </div>
            <button @click="openModal()" class="btn btn-danger shadow-sm text-nowrap w-100 w-md-auto">
                <i class="bi bi-plus-lg me-2"></i> Input Inpassing
            </button>
        </div>
            ...
            <div class="card-header bg-white p-3 border-bottom-0">
                <div class="row g-2 align-items-center">
                    ...
                    <div class="col-12 col-md-auto d-flex gap-2 align-items-center">
                        ...
                        <button @click="fetchTable(1)" class="btn btn-sm btn-outline-danger" title="Filter"><i class="bi bi-funnel-fill"></i></button>
                    </div>
                        ...
                        <table class="table table-hover align-middle mb-0 text-nowrap" style="min-width: 1000px;">
                            <thead class="bg-light">
                                <tr>
                                    ...
                                    <th class="py-3 small fw-bold">Gaji Inpassing</th>
                                    ...
                                    <tr :class="{'bg-light': isExpanded(item.id)}">
                                        <td class="text-center">
                                            <button class="btn btn-sm btn-link text-decoration-none p-0" @click="toggleRow(item.id)">
                                                <i class="bi" :class="isExpanded(item.id) ? 'bi-dash-circle-fill text-danger' : 'bi-plus-circle-fill text-secondary'"></i>
                                            </button>
                                        </td>
                                        <td class="ps-2">
                                    ...
                                    <tr v-if="isExpanded(item.id)" class="bg-light border-bottom fade-in">
                                        <td colspan="6" class="p-3 ps-5">
                                            <div class="row g-3 small">
                                                ...
                                                <div class="col-md-4">
                                                    <h6 class="fw-bold text-muted mb-2 text-uppercase" style="font-size: 0.7rem;">Data Inpassing</h6>
                                                    <table class="table table-sm table-borderless mb-0 w-auto">
                                                        <tr><td class="text-muted ps-0">Masa Kerja:</td><td class="fw-bold">{{ item.mk_baru_tahun }} Tahun {{ item.mk_baru_bulan }} Bulan</td></tr>
                                                        <tr><td class="text-muted ps-0">TMT YAD:</td><td class="text-danger fw-bold">{{ item.tmt_selanjutnya ? formatTanggal(item.tmt_selanjutnya) : '-' }}</td></tr>
                                                    </table>
                                                </div>
                                                <div class="col-md-4">
                        ...
                        <div v-for="item in listData" :key="'mob-'+item.id" class="card border-0 shadow-sm">
                            <div class="card-body">
                                <div class="d-flex justify-content-between align-items-start mb-2">
                                    <div><h6 class="fw-bold mb-0 text-danger">{{ item.nama_snapshot }}</h6><small class="text-muted font-monospace">{{ item.nip }}</small></div>
                                    <span class="badge rounded-pill" :class="{'bg-secondary': !item.status || item.status==='DRAFT', 'bg-warning text-dark': item.status==='TEKEN', 'bg-success': item.status==='DISTRIBUSI'}">{{ item.status || 'DRAFT' }}</span>
                                </div>
        ...
        <div class="modal-dialog modal-xl modal-fullscreen-sm-down modal-dialog-scrollable">
            <div class="modal-content border-0 shadow-lg">
                <div class="modal-header bg-danger text-white">
                    <h5 class="modal-title fw-bold">{{ isEditMode ? 'Edit Inpassing' : 'Input Inpassing Baru' }}</h5>
                    <button type="button" class="btn-close btn-close-white" @click="closeModal"></button>
                </div>
                <div class="modal-body bg-light p-3 p-md-4">
                    <form @submit.prevent="simpanTransaksi">
                        <div class="card shadow-sm border-0 mb-3">
                            <div class="card-header bg-white py-3"><h6 class="fw-bold text-danger mb-0">1. Identitas Pegawai</h6></div>
                            <div class="card-body">
                                <div class="row g-3">
