    ...
    setup() {
        ...
        const fetchReminder = async () => {
            ...
            try {
                ...
                // Query Reminder (Next KGB) dari Jantung Utama Master Pegawai
                const constraints = [
                    where("tmt_kgb_berikutnya", ">=", todayStr),
                    where("tmt_kgb_berikutnya", "<=", futureStr),
                    orderBy("tmt_kgb_berikutnya", "asc"),
                    limit(50) // Naikkan sedikit kapastias
                ];

                // Note: Karena tabel Master Pegawai adalah tabel tunggal tanpa 'created_by', 
                // semua TU / Admin akan melihat Alarm yang sama. (Transparansi proaktif).

                const q = query(collection(db, "master_pegawai"), ...constraints);
                const snap = await getDocs(q);

                listReminder.value = snap.docs.map(d => {
                    const data = d.data();
                    // Mapping ulang struktur agar cocok dgn UI sebelumnya tanpa modif HTML
                    return {
                        id: d.id,
                        nip: data.nip,
                        nama: data.nama,
                        nama_snapshot: data.nama,
                        tmt_selanjutnya: data.tmt_kgb_berikutnya
                    };
                });
                ...
                if (e.message.includes("index")) {
                    console.warn("Butuh Index di master_pegawai: tmt_kgb_berikutnya (asc)");
                }
            }
