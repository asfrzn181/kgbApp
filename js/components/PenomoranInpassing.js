...
export default {
    ...
    setup() {
        ...
        const form = reactive({
            ...
            keterangan_inpassing: '', perpres_inpassing: '',
        ...
        const handleUsulanChange = () => {
            ...
            if (selected) {
                ...
                if (!isEditMode.value) {
                    ...
                    form.perpres_inpassing = selected.perpres || '';
        ...
        const simpanFinal = async () => {
            ...
            try {
                ...
                if (match) currentUrut = parseInt(match[1]);
                else {
                ...
                const dataUpdateUtama = {
                    ...
                    perpres_inpassing: form.perpres_inpassing || ''
                };
        ...
        const editNomor = async (item) => {
            ...
            Object.assign(form, {
                ...
                perpres_inpassing: item.perpres_inpassing || '',
        ...
        const generateDocBlob = async (usulanId) => {
            ...
            docRender.render({
                ...
                PERPRES: item.perpres_inpassing || "-",
        ...
        const openModal = async () => {
            ...
            Object.assign(form, {
                ...
                keterangan_inpassing: '', perpres_inpassing: '', tanggal_inpassing_manual: getTodayISO(), tmt_inpassing: getNextTwoMonthsISO()
            });
            customNumberStatus.value = null; customNumberMsg.value = '';
            await fetchUsulanList(); showModal.value = true;
        };
        ...
        const initRefs = async () => {
            try {
                const s = await getDocs(collection(db, "master_gaji"));
                const pSet = new Set();
                s.forEach(d => { if (d.data().perpres) pSet.add(d.data().perpres); });
                listPerpres.value = Array.from(pSet).map(p => ({ judul: p }));
            } catch (e) { }
        };

        onMounted(() => { fetchData(1); initRefs(); });

        return {
            listData, listUsulan, listPerpres, loading, showModal, isSaving, form, isEditMode, yearOptions, previewTab,
