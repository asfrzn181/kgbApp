...
export default {
    ...
    template: TplPenomoran, // Menggunakan Template Utama dari View
    setup() {
        // STATE LIST DATA
        const listData = ref([]);
        ...
        const form = reactive({
            ...
            nomor_custom: '', 
            no_urut: 0,
            kategori: 'INPASSING' // Flag Khusus
        });
        ...
        // --- FETCH DATA LOG (FILTER: INPASSING) ---
        const fetchData = async (pageTarget) => {
            ...
            try {
                ...
                const constraints = [];
                // FILTER WAJIB: KATEGORI INPASSING
                constraints.push(where("kategori", "==", "INPASSING")); 

                if (filterStartDate.value) {
                    constraints.push(where("created_at", ">=", new Date(filterStartDate.value)));
                }
        ...
        const fetchUsulanList = async () => {
            try {
                ...
                // TARGET COLLECTION: usulan_inpassing
                const q = query(collection(db, "usulan_inpassing"), ...constraints);
        ...
        const handleUsulanChange = () => {
            ...
            if (selected) {
                ...
                if(!isEditMode.value) {
                    form.golongan = selected.golongan_baru || selected.golongan; 
                    const j = (selected.jabatan_baru || '').toLowerCase();
                    // Logika deteksi Struktural sederhana
                    if (j.includes('kepala') || j.includes('kabid') || j.includes('camat') || j.includes('sekretaris')) {
            ...
            try {
                ...
                if (!snap.empty) {
                    if (isEditMode.value && snap.docs[0].id === editId.value) {
                        ...
                        customNumberMsg.value = `Dipakai: ${owner.nama_pegawai}`;
                    }
                } else {
                    // Validasi Kode Inpassing (Contoh: 800.1.2)
        ...
        const previewNomor = async () => {
            ...
            try {
                // Counter ID: 2024_STRUKTURAL atau 2024_FUNGSIONAL
                ...
                // KODE KLASIFIKASI INPASSING (Sesuaikan dengan Perbup)
                // Contoh: 800.1.2.2 (Pengangkatan Perpindahan Jabatan)
                const kodeKlasifikasi = "800.1.2.2"; 
                
                form.nomor_custom = `B-${kodeKlasifikasi}/${noUrutStr}/BKPSDMD/${form.tahun}`;
                ...
                if (isEditMode.value) {
                    await runTransaction(db, async (transaction) => {
                        ...
                        if (oldUsulanId.value && oldUsulanId.value !== form.usulan_id) {
                            const oldRef = doc(db, "usulan_inpassing", oldUsulanId.value);
                            transaction.update(oldRef, { nomor_naskah: null, tanggal_naskah: null });
                        }
                        const newRef = doc(db, "usulan_inpassing", form.usulan_id);
                    ...
                    // SIMPAN KE LOG: Tandai Kategori INPASSING
                    ...
                    await updateDoc(doc(db, "usulan_inpassing", form.usulan_id), { nomor_naskah: form.nomor_custom, tanggal_naskah: serverTimestamp() });
            ...
            if (await showConfirm('Batalkan Nomor?', 'Counter akan mundur jika nomor terakhir.')) {
                try {
                    ...
                    await runTransaction(db, async (transaction) => {
                        ...
                        if(item.usulan_id) transaction.update(doc(db, "usulan_inpassing", item.usulan_id), { nomor_naskah: null, tanggal_naskah: null });
                    });
        ...
        const generateDocBlob = async (usulanId) => {
            ...
            const snap = await getDoc(doc(db, "usulan_inpassing", usulanId));
            if(!snap.exists()) throw new Error("Data Usulan tidak ditemukan!");
            const item = snap.data();

            let pangkatFinal = item.pangkat_baru || item.pangkat || ""; 

            // Template ID: INPASSING
            const ts = await getDoc(doc(db, "config_template", "INPASSING")); 
            if(!ts.exists()) throw new Error("Template INPASSING Belum Diupload!");
            ...
            const golKode = item.golongan_baru || "";
            ...
            let targetNip = isSetda ? gvd.kop_setda?.pejabat_nip : gvd.kop_bkpsdmd?.pejabat_nip;
            ...
            docRender.render({
                ...
                NIP: item.nip || "",
                PANGKAT_LAMA: item.pangkat_lama || "-",
                GOLONGAN_LAMA: item.golongan_lama || "-",
                JABATAN_LAMA: item.jabatan_lama || "-",
                PANGKAT_BARU: item.pangkat_baru || pangkatFinal,
                GOLONGAN_BARU: item.golongan_baru || "-",
                JABATAN_BARU: item.jabatan_baru || "-",
                ANGKA_KREDIT: item.angka_kredit || "0", 
                UNIT_KERJA: item.unit_kerja,
                TMT_INPASSING: formatTanggal(item.tmt_inpassing || new Date()),
                KOP: kopT, ALAMAT_KOP: kopA, 
                NOMOR_NASKAH: item.nomor_naskah||"....................", 
                TANGGAL_NASKAH: tanggalSurat, 
                TTD_PENGIRIM: ttdContent, 
                JABATAN_PEJABAT: pjj, PANGKAT_PEJABAT: pjp, 
        ...
        const cetakSK = async (logItem) => {
            try {
                ...
                const prefix = previewTab.value === 'TTE' ? 'DRAFT_TTE_' : 'SK_INPASSING_';
        ...
        const openModal = async () => {
            ...
            Object.assign(form, { usulan_id:'', nama_pegawai:'', nip:'', jenis_jabatan:'Fungsional', golongan:'', tahun: new Date().getFullYear(), nomor_custom:'', no_urut: 0, kategori:'INPASSING' });
            customNumberStatus.value = null; customNumberMsg.value = '';
            await fetchUsulanList(); showModal.value = true;
        };
        ...
        // RETURN SEMUA VARIABEL AGAR DIBACA TEMPLATE VIEW
        return {
