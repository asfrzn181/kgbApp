...
import {
    ...
    formatRupiah, formatTanggal, formatTitleCase,
    fetchWithCache
} from '../utils.js';
...
const AutocompleteJabatan = {
    ...
    setup(props, { emit }) {
        ...
        const handleInput = (e) => {
            ...
            // Saat menghapus (Backspace/Delete/cut/dll): jangan format, biarkan user hapus bebas
            ...
            // Saat mengetik: terapkan Title Case sambil jaga posisi kursor
        ...
        // Format sekali saat user selesai (keluar dari field)
...
const AutocompleteUnitKerja = {
    ...
    setup(props, { emit }) {
        ...
        const handleInput = (e) => {
            const raw = e.target.value;
            // Saat menghapus: jangan format, biarkan user hapus bebas
            if (e.inputType && e.inputType.startsWith('delete')) {
                emit('update:modelValue', raw);
                fetchSuggestions(raw);
                showSuggestions.value = true;
    ...
    setup(props, { emit }) {
        ...
        const initRefs = async () => {
            ...
            if (listGolongan.value.length === 0) {
                ...
                const dataGol = await fetchWithCache('KGB_MASTER_GOLONGAN', qGol, 12);
                listGolongan.value = [];
            ...
            if (listPejabat.value.length === 0) {
                ...
                const dataPj = await fetchWithCache('KGB_MASTER_PEJABAT', qPj, 12);
            ...
            if (!cacheGlobalVars.value.dasar_hukum) {
                ...
                const dataConfig = await fetchWithCache('KGB_GLOBAL_VARS', docRef, 12);
                if (dataConfig) {
                    cacheGlobalVars.value = dataConfig; // SIMPAN DI CACHE VUE
                    if (dataConfig.dasar_hukum) listDasarHukum.value = dataConfig.dasar_hukum;
                    if (dataConfig.kop_setda) configPejabat.setda = dataConfig.kop_setda.pejabat_nip;
                    if (dataConfig.kop_bkpsdmd) configPejabat.bkpsdmd = dataConfig.kop_bkpsdmd.pejabat_nip;
                }
            }

            if (listPerpres.value.length === 0) {
                const qPerpres = collection(db, "master_gaji"); // Walaupun banyak, kita hit 1x saja seumur hidup per 12 jam
                const dataGaji = await fetchWithCache('KGB_MASTER_GAJI_ALL', qPerpres, 12);
                const pSet = new Set();
                dataGaji.forEach(data => { if (data.perpres) pSet.add(data.perpres); });
                listPerpres.value = Array.from(pSet).map(p => ({ judul: p }));
            }
        };

        const getGaji = async (gol, mk, perpres_val) => {
            if (!gol) return { gaji: 0, perpres: '' };
            try {
                let qConstraints = [where("golongan", "==", gol), where("mkg", "==", Number(mk || 0))];
                if (perpres_val) qConstraints.push(where("perpres", "==", perpres_val));

                const q = query(collection(db, "master_gaji"), ...qConstraints, limit(1));
                const s = await getDocs(q);
                return !s.empty ? { gaji: s.docs[0].data().gaji, perpres: s.docs[0].data().perpres || '' } : { gaji: 0, perpres: perpres_val || '' };
            } catch { return { gaji: 0, perpres: perpres_val || '' }; }
        };
        let reqIdGajiBaru = 0;
        const cariGajiBaru = async () => {
            const currentReqId = ++reqIdGajiBaru;
            const res = await getGaji(form.golongan, form.mk_baru_tahun, form.perpres);
            if (currentReqId !== reqIdGajiBaru) return; // Ignore stale response
            form.gaji_baru = res.gaji;
            if (form.gaji_baru === 0) gajiMsg.value = 'Gaji not found'; else gajiMsg.value = '';
        };

        let reqIdGajiLama = 0;
        const cariGajiLama = async () => {
            const currentReqId = ++reqIdGajiLama;
            const res = await getGaji(form.dasar_golongan, form.dasar_mk_tahun, form.perpres);
            if (currentReqId !== reqIdGajiLama) return; // Ignore stale response
            form.dasar_gaji_lama = res.gaji;
        };

        const debouncedCariGajiBaru = debounce(cariGajiBaru, 500);
        const debouncedCariGajiLama = debounce(cariGajiLama, 500);

        watch(() => [form.golongan, form.mk_baru_tahun, form.perpres], debouncedCariGajiBaru);
        watch(() => [form.dasar_golongan, form.dasar_mk_tahun], debouncedCariGajiLama);
        ...
        const extractTglLahir = (nip) => {
            ...
            // Validasi angka dan range tanggal yang masuk akal
            if (!isNaN(y) && !isNaN(m) && !isNaN(d) && m >= '01' && m <= '12' && d >= '01' && d <= '31') {
                return `${y}-${m}-${d}`; // Format YYYY-MM-DD untuk input type=date
            }
            return '';
        };
        ...
        const handleNipInput = debounce(async (e) => {
            ...
            try {
                ...
                if (snap.exists()) {
                    const d = snap.data();
                    // Tanggal lahir: NIP lebih reliable (mengandung tgl lahir di 8 digit pertama)
                    // Gunakan NIP sebagai sumber utama, fallback ke database jika NIP tidak valid
                    const finalTgl = dateFromNip || d.tgl_lahir || '';
                    const rawJenis = String(d.jenis_jabatan || '').toLowerCase();
                    let finalJenis = 'Pelaksana';
                    if (rawJenis.includes('fungsional')) finalJenis = 'Fungsional';
                    else if (rawJenis.includes('struktur')) finalJenis = 'Struktural';
                    ...
                    Object.assign(form, {
                        ...
                        jenis_jabatan: formatTitleCase(String(d.jenis_jabatan || 'Pelaksana').trim())
                    });
        ...
        const checkBup = () => {
            if (form.tgl_lahir) {
                const birth = new Date(form.tgl_lahir);
            ...
            if (isNaN(bd.getTime()) || isNaN(tmt.getTime())) return;
        ...
        const handleJabatanSelect = (item) => {
            ...
            // Kita ambil langsung dari database, lalu pastikan kapitalisasinya
            // Format Title Case ('Fungsional' / 'Struktural' / 'Pelaksana') agar dropdown merespon
            form.jenis_jabatan = formatTitleCase(String(item.jenis_jabatan || 'Pelaksana').trim());

            currentBup.value = item.bup || 58;
            checkBup();
        };

        const handleGolonganLamaChange = (kode) => {
            const selected = listGolongan.value.find(g => g.kode === kode);
            if (selected) {
                form.golongan = selected.kode;
