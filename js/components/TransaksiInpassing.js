...
// --- IMPORT VIEW (Pastikan path benar) ---
import { TplSearchSelect, TplAutocompleteJabatan, TplAutocompleteUnitKerja, TplAutocompletePerangkatDaerah, TplMain } from '../views/TransaksiInpassingView.js';
...
// AUTOCOMPLETE UNIT KERJA (FROM INPASSING HISTORY)
const AutocompleteUnitKerja = {
    ...
    setup(props, { emit }) {
        ...
        const processSnap = (snap, keyword) => {
            ...
            snap.forEach(doc => { const v = doc.data().unit_kerja; if(v && v.toLowerCase().includes(keyLower)) units.add(v); });
            return Array.from(units).slice(0, 5);
        };
        const fetchSuggestions = debounce(async (keyword) => {
            if (!keyword || keyword.length < 3) { suggestions.value = []; return; }
            try {
                const qGlobal = query(collection(db, "usulan_inpassing"), orderBy("created_at", "desc"), limit(50)); 
                const snap = await getDocs(qGlobal);
                suggestions.value = processSnap(snap, keyword);
            } catch (e) {
                if (auth.currentUser) {
                    try {
                        const qOwn = query(collection(db, "usulan_inpassing"), where("created_by", "==", auth.currentUser.uid), orderBy("created_at", "desc"), limit(50));
...
// AUTOCOMPLETE PERANGKAT DAERAH (FROM INPASSING HISTORY)
const AutocompletePerangkatDaerah = {
    ...
    setup(props, { emit }) {
        ...
        const processSnap = (snap, keyword) => {
            ...
            const keyLower = keyword.toLowerCase();
            snap.forEach(doc => { const v = doc.data().perangkat_daerah; if(v && v.toLowerCase().includes(keyLower)) results.add(v); });
...
export default {
    ...
    setup() {
        ...
        const fetchTable = async (pageTarget) => {
            ...
            try {
                // [NOTE] COLLECTION BERBEDA
                const collRef = collection(db, "usulan_inpassing"); 
        ...
        const goToPage = (p) => {
            ...
            if (p === currentPage.value + 1 || p === currentPage.value - 1) fetchTable(p);
            else fetchTable(1);
        };
        ...
        const updateStatus = async (item, newStatus) => {
            ...
            try {
                ...
                // [NOTE] COLLECTION BERBEDA
                await updateDoc(doc(db, "usulan_inpassing", item.id), updateData);
        ...
        const simpanTransaksi = async () => {
            ...
            try {
                ...
                if(isEditMode.value) {
                    // [NOTE] COLLECTION BERBEDA
                    await updateDoc(doc(db,"usulan_inpassing",formId.value), payload);
                } else { 
                    payload.created_at=serverTimestamp(); payload.created_by=auth.currentUser.uid; payload.status='DRAFT'; 
                    // [NOTE] COLLECTION BERBEDA
                    await addDoc(collection(db,"usulan_inpassing"), payload); 
                }
        ...
        const hapusTransaksi = async(item)=>{ 
            ...
            if(await showConfirm("Hapus?","Data hilang.")) { 
                // [NOTE] COLLECTION BERBEDA
                await deleteDoc(doc(db,"usulan_inpassing",item.id)); 
                fetchTable(1); 
            } 
        };
        ...
        const generateDocBlob = async (item) => {
            ...
            // [NOTE] TEMPLATE KHUSUS INPASSING
            const tplId = item.tipe_asn === 'PPPK' ? "INPASSING_PPPK" : "INPASSING_PNS"; 
            const ts = await getDoc(doc(db, "config_template", tplId)); 
            if(!ts.exists()) throw new Error("Template Inpassing Belum Diupload! (ID: " + tplId + ")");
