...
export const siasnBookmarklet = `javascript:(function(){var p=new URLSearchParams(window.location.search);var nip=p.get('kgb_nip');var origin=p.get('kgb_origin')||'*';if(!nip){alert('Buka dari KGB App dulu! Klik tombol Cek SIASN di form SK Fungsional, jangan buka SIASN manual.');return;}function s(el,v){if(!el)return;var ns=Object.getOwnPropertyDescriptor(window.HTMLInputElement.prototype,'value').set;ns.call(el,v);el.dispatchEvent(new Event('input',{bubbles:true}));el.dispatchEvent(new Event('change',{bubbles:true}));}function gv(lbl){var ls=document.querySelectorAll('label');for(var i=0;i<ls.length;i++){if(ls[i].textContent.trim().toLowerCase().includes(lbl.toLowerCase())){var pe=ls[i].parentElement;if(pe){var el=pe.querySelector('p');if(el)return el.textContent.trim();}}}return'';}function pd(str){if(!str||str==='-')return'';var m={'januari':'01','februari':'02','maret':'03','april':'04','mei':'05','juni':'06','juli':'07','agustus':'08','september':'09','oktober':'10','november':'11','desember':'12'};var pts=str.toLowerCase().trim().split(/\s+/);if(pts.length<3)return'';return pts[2]+'-'+(m[pts[1]]||'01')+'-'+pts[0].padStart(2,'0');}function extract(){var nama=gv('Nama');var golR=gv('Golongan Ruang Akhir')||gv('Golongan Ruang')||gv('Golongan');var tmtG=gv('TMT Golongan')||gv('TMT Gol');var unit=gv('Unit Organisasi')||gv('Unit Kerja');var induk=gv('Unit Organisasi Induk')||gv('Perangkat Daerah')||gv('Satuan Kerja');var pkt=gv('Pangkat');var gm=golR.match(/([IVX]+\/[a-d])/i);var gk=gm?gm[1].toUpperCase():golR;var pg=pkt&&gk?pkt+' / '+gk:golR;var payload={nama:nama,unit_kerja:unit,perangkat_daerah:induk,pangkat_golongan:pg,golongan_kode:gk,tmt_pangkat_golongan:pd(tmtG)};if(window.opener){window.opener.postMessage({type:'KGBAPP_SIASN_DATA',payload:payload},decodeURIComponent(origin));alert('\u2705 Data dikirim ke KGB App!\n\nNama: '+nama+'\nGolongan: '+pg+'\nUnit: '+unit+'\n\nTab ini bisa ditutup.');}else{navigator.clipboard.writeText(JSON.stringify(payload)).then(function(){alert('Data disalin ke clipboard (window.opener tidak tersedia).');}).catch(function(){alert('Payload: '+JSON.stringify(payload));});}}var nipInp=document.querySelector('input[name="nip_nama"]')||document.querySelector('input[placeholder*="NIP"]');if(!nipInp){alert('Field NIP tidak ditemukan. Pastikan di halaman SIASN yang benar.');return;}var btnNip=Array.from(document.querySelectorAll('button')).find(function(b){return b.textContent.trim()==='NIP';});if(btnNip&&!btnNip.className.includes('bg-secondaryBkn'))btnNip.click();s(nipInp,nip);setTimeout(function(){var btnCari=Array.from(document.querySelectorAll('button')).find(function(b){return b.textContent.trim()==='Cari';});if(btnCari){btnCari.click();setTimeout(extract,2800);}else{alert('Tombol Cari tidak ditemukan.');extract();}},400);})();`;